...
def loss_KL(student_att, teacher_att):
    #student_att  [1,12,seq_len,seq_len]
    #teacher_att  [1,64,seq_len,seq_len]
    bs, num_heads_s= student_att.size()[0],student_att.size()[1]

    bs, num_heads_t = teacher_att.size()[0],teacher_att.size()[1]
    
    new_student_att = student_att.reshape(bs, num_heads_s, -1)   #  [1,12,seq_len*seq_len]

    new_teacher_att = teacher_att.reshape(bs, num_heads_s, -1)   #  [1,64,seq_len*seq_len]
    
    new_teacher_att = new_teacher_att.transpose(-2,-1)    # [1,seq_len*seq_len,64]

    mixed_attention=  torch.matmul(new_student_att,new_teacher_att)  # [1, 12, 64]
    
    mixed_attention_softmax = F.softmax(mixed_attention,dim=-1)

    mixed_attention_drop_bs= mixed_attention_softmax.squeeze()  # [12,64]
    
    teacher_head_index=torch.argmax(mixed_attention_drop_bs,dim=-1).tolist()   

    teacher_att_selected =  teacher_att[:,teacher_head_index]  # (bs, 12, seq_len,seq_len)
...
for epoch in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        new_student_atts =  [student_atts[i] for i in range(student_layer_num) if i%2==1]
        new_teacher_atts = [teacher_atts[i] for i in range(teacher_layer_num) if i %2==1]
        ...
        for student_att, teacher_att in zip(student_atts, new_teacher_atts):
            student_att = torch.where(student_att <= -1e3, torch.zeros_like(student_att).to(args.device),
                                        student_att)
            teacher_att = torch.where(teacher_att <= -1e3, torch.zeros_like(teacher_att).to(args.device),
                                        teacher_att)

            att_loss += loss_KL(student_att, teacher_att)
        ...
        graph_state_loss= d_loss[6]  #0.023  由于这个值非常小单独拿出来
        ...
        tol_loss = d_loss/d_loss.detach()+ loss_ditill/loss_ditill.detach() + graph_state_loss/graph_state_loss.detach() + loss_hard/loss_hard.detach()
